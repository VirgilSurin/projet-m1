...
Cette section vous présente un exemple de l'exécution de CLIQUES sur le graphe en Figure \ref{fig:exemple}.

Nous commençons par le premier appel :
\begin{itemize}
  \item \texttt{SUBG} = \texttt{CAND} = \(\{1,2,3,4,5,6,7,8,9\}\)
  \item \texttt{Q} est vide
\end{itemize}

Le premier appel se fait avec ses argument. \texttt{SUBG} n'étant pas vide, nous allons choisir un pivot \emph{u}. Pour rappel, nous cherchons à trouver le noeud dans \texttt{SUBG} tel que l'intersection entre les voisins de ce noeuds et \texttt{CAND} est le plus grand possible. Dans notre exemple il s'agit de 4.
