...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \node[fill=blue!50] (n4) at (-2, 2) {4};
    \node[fill=blue!50] (n5) at (-2, 4) {5};
    \node[fill=blue!50] (n6) at (-2, 6) {6};

    \node[fill=red!50] (n7) at (0, 8) {7};
    \node[fill=red!50] (n8) at (2, 8) {8};
    \node[fill=red!50] (n9) at (4, 8) {9};
...
En observant les temps obtenu pour les différents algorithmes, nous constatons que \texttt{BKP_M} (avec pour pivot le noeud de degré maximum) semble se comporter mieux de manière générale.
